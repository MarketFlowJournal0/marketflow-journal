  ...
  useEffect(() => {
    ...
    function init() {
      ...
      for (let i = 0; i < LINE_COUNT; i++) {
        const pts = [];
        let x = Math.random() * (w / window.devicePixelRatio);
    ...
    function draw() {
      ...
      lines.forEach(line => {
        line.pts.forEach(p => { p.x += line.speed * 0.3; if (p.x > rw + 50) { p.x = -50; p.y = Math.random() * rh; } });
        ctx.beginPath();
        ctx.moveTo(line.pts[0].x, line.pts[0].y);
        for (let i = 1; i < line.pts.length; i++) {
          const prev = line.pts[i - 1];
          const curr = line.pts[i];
...
const STYLES = `
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .lp-hero-badge {
    ...
    font-size:11px;font-weight:700;color:var(--cyan);letter-spacing:0.5px;margin-bottom:28px;
  }
  ...
  .lp-hero h1 {
    ...
    font-size:clamp(44px,6vw,82px);line-height:1.05;
    color:var(--t0);letter-spacing:-2px;margin-bottom:22px;
    max-width:900px;
  }
  ...
  .btn-hero-primary {
    ...
    border:none;color:#030508;font-size:15px;font-weight:800;
    cursor:pointer;font-family:'Inter',sans-serif;
    transition:all 0.2s;box-shadow:0 0 40px rgba(6,230,255,0.3);
    display:flex;align-items:center;gap:8px;
  }
  .btn-hero-primary:hover { transform:translateY(-2px);box-shadow:0 8px 50px rgba(6,230,255,0.45); }
  .btn-hero-secondary {
    padding:15px 30px;border-radius:12px;
    border:1px solid var(--brd);background:rgba(255,255,255,0.03);
  ...
  .lp-hero-stats {
    ...
    background:rgba(12,20,34,0.6);backdrop-filter:blur(20px);
    overflow:hidden;
  }
  ...
  .lp-compare-table { width:100%;border-collapse:separate;border-spacing:0; }
  .lp-compare-table th { padding:12px 16px;font-size:10px;font-weight:800;color:var(--t3);letter-spacing:1px;text-transform:uppercase;text-align:left;border-bottom:1px solid var(--brd); }
  .lp-compare-table th.hl { color:var(--cyan); }
  .lp-compare-table td { padding:12px 16px;font-size:13px;color:var(--t2);border-bottom:1px solid rgba(255,255,255,0.03); }
  ...
  .lp-pricing-card { padding:28px 24px;border-radius:18px;border:1px solid var(--brd);background:rgba(12,20,34,0.5);position:relative;overflow:hidden;transition:all 0.3s; }
  ...
  .lp-save { display:inline-block;padding:2px 8px;border-radius:4px;font-size:9px;font-weight:800;background:rgba(0,255,136,0.1);border:1px solid rgba(0,255,136,0.2);color:var(--green);margin-bottom:20px; }
  .lp-divider { height:1px;background:var(--brd);margin:18px 0; }
  .lp-price-feats { list-style:none;display:flex;flex-direction:column;gap:9px;margin-bottom:24px; }
...
// ─── Live Ticker ───────────────────────────────────────────────────────────
const FALLBACK = {
  'BTC/USD':{p:83200,pct:1.24},'ETH/USD':{p:2010,pct:0.87},'SOL/USD':{p:132,pct:1.45},
  'EUR/USD':{p:1.0842,pct:0.12},'GBP/USD':{p:1.271,pct:0.21},'USD/JPY':{p:149.82,pct:-0.08},
  'S&P 500':{p:5680,pct:0.34},'NASDAQ':{p:19800,pct:0.56},'DOW':{p:41500,pct:0.22},
};
    ...
    <div className="lp-ticker-wrap">
      ...
      <div className="lp-ticker">
        {doubled.map((name,i)=>{
          ...
          const pos=d.pct>=0;
          return (
            <span key={`${name}-${i}`} style={{display:'inline-flex',alignItems:'center',gap:8,padding:'0 18px',borderRight:'1px solid rgba(255,255,255,0.03)'}}>
              <span style={{color:'#3A5070',fontFamily:"'JetBrains Mono',monospace",fontSize:9,fontWeight:600,letterSpacing:'1px'}}>{name}</span>
              <span style={{color:'#D8E8FF',fontFamily:"'JetBrains Mono',monospace",fontSize:11.5,fontWeight:500}}>{fmt(name,d.p)}</span>
              <span style={{display:'inline-flex',alignItems:'center',gap:3,padding:'2px 6px',borderRadius:4,background:pos?'rgba(0,192,112,0.08)':'rgba(255,68,85,0.08)',border:`1px solid ${pos?'rgba(0,192,112,0.15)':'rgba(255,68,85,0.15)'}`,fontFamily:"'JetBrains Mono',monospace",fontSize:9,fontWeight:700,color:pos?'#00C070':'#FF4455'}}>{pos?'▲':'▼'}{Math.abs(d.pct).toFixed(2)}%</span>
            </span>
          );
        })}
      </div>
...
// ─── Page Modal ────────────────────────────────────────────────────────────
const PAGE_CONTENT = {
  changelog:{title:'📋 Changelog',subtitle:'Update History',color:'#06E6FF',items:[{v:'v2.5.0',date:'April 2026',badge:'LATEST',badgeColor:'#00FF88',items:['✅ Premium landing page redesign','✅ Broker Connect — MT4/MT5 auto-sync','✅ AI ChatBot floating assistant','✅ Rank system with country/world rankings']},{v:'v2.4.0',date:'March 2026',badge:'',badgeColor:'',items:['✅ Trading calendar with MarketFlow Rank','✅ Light/dark theme','✅ Pro Analytics with advanced indicators','✅ Psychology module','✅ Strategy backtesting']},{v:'v2.3.0',date:'February 2026',badge:'',badgeColor:'',items:['✅ Stripe subscriptions','✅ Secure Supabase auth','✅ Redesigned dashboard','✅ Live market ticker']}]},
    ...
    <div className="lp-modal-overlay" onClick={onClose}>
      <div className="lp-modal" onClick={e=>e.stopPropagation()}>
        <div className="lp-modal-header">
          <div><h3 style={{fontSize:18,fontWeight:800,color:'#fff',margin:0}}>{content.title}</h3><p style={{fontSize:12,color:'#7A90B8',margin:'4px 0 0'}}>{content.subtitle}</p></div>
        ...
        <div className="lp-modal-body">
          {content.items?.map((item,i) => (
            <div key={i} style={{marginBottom:20}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                <span style={{fontSize:13,fontWeight:800,color:content.color}}>{item.v||item.label}</span>
                {item.badge && <span style={{padding:'2px 8px',borderRadius:4,fontSize:8,fontWeight:800,background:`${item.badgeColor}15`,border:`1px solid ${item.badgeColor}30`,color:item.badgeColor}}>{item.badge}</span>}
                <span style={{fontSize:11,color:'#334566'}}>{item.date}</span>
              </div>
              {(item.items||[]).map((t,j)=><div key={j} style={{fontSize:13,color:'#7A90B8',padding:'4px 0 4px 16px',lineHeight:1.6}}>{t}</div>)}
            </div>
          ))}
          {content.sections?.map((sec,i) => (
            <div key={i} style={{marginBottom:20}}>
              <div style={{fontSize:13,fontWeight:700,color:sec.color,marginBottom:10}}>{sec.label}</div>
              {(sec.items||[]).map((item,j) => (
                <div key={j} style={{display:'flex',gap:10,marginBottom:12}}>
                  <span style={{fontSize:16}}>{item.icon}</span>
                  <div><div style={{fontSize:13,fontWeight:700,color:'#E8EEFF'}}>{item.title}</div><div style={{fontSize:12,color:'#7A90B8',lineHeight:1.55}}>{item.desc}</div></div>
                </div>
              ))}
            </div>
          ))}
          {content.articles?.map((a,i) => (
            <div key={i} style={{marginBottom:16}}>
              <div style={{fontSize:13,fontWeight:700,color:'#E8EEFF',marginBottom:4}}>{a.title}</div>
              <div style={{fontSize:12.5,color:'#7A90B8',lineHeight:1.65}}>{a.text}</div>
            </div>
          ))}
          {content.content && <div style={{fontSize:13,color:'#7A90B8',lineHeight:1.7,whiteSpace:'pre-wrap'}}>{content.content}</div>}
        </div>
      </div>
    </div>
  );
    ...
    <div style={{background:'#030508',minHeight:'100vh',overflowX:'hidden'}}>
      ...
      <nav className={`lp-nav ${scrolled?'scrolled':''}`}>
        ...
        <div className="lp-nav-cta">
          ...
          <button className="btn-primary-nav" onClick={onSignup}>Start free trial →</button>
        </div>
      </nav>
      ...
      <section className="lp-hero">
        ...
        <Reveal delay={0.2}>
          <div className="lp-hero-actions">
            <button className="btn-hero-primary" onClick={onSignup}>Start your free trial <span>→</span></button>
        ...
        <Reveal delay={0.4}>
          <div className="lp-hero-stats">
            ...
            <div className="lp-hero-stat"><div className="lp-stat-val"><Counter end={4.9} /></div><div className="lp-stat-label">Avg Rating</div></div>
          </div>
        </Reveal>
      </section>
      ...
      <div className="lp-logos">
        <p>Used by traders from</p>
        <div className="lp-logos-row">
          {['FTMO','The5%ers','E8 Funding','TopStep','MyFundedFX','True Forex Funds'].map(n=><span key={n} className="lp-logo-item">{n}</span>)}
        </div>
      </div>
      ...
      <section className="lp-section" id="features">
        <div className="lp-section-inner">
          ...
          <div className="lp-features-grid">
            {[
              {icon:'📊',title:'Smart Trade Journal',desc:'Log trades in seconds. Auto-import from MT4, MT5, cTrader or any CSV. Every detail captured automatically.'},
              {icon:'🤖',title:'AI Trade Coach',desc:'Your personal trading AI analyzes patterns, detects biases, and gives actionable recommendations to improve.'},
              {icon:'📈',title:'Advanced Analytics',desc:'Sharpe ratio, profit factor, expectancy, drawdown analysis — all the metrics pros use in one dashboard.'},
              {icon:'🧠',title:'Psychology Tracker',desc:'Track your emotional state, identify tilt patterns, and understand how your mindset affects your P&L.'},
              {icon:'🔄',title:'Strategy Backtesting',desc:'Test your strategies on historical data with visual charts. Monte Carlo simulation for risk analysis.'},
              {icon:'🏆',title:'Prop Firm Ready',desc:'Formatted reports for FTMO, The5%ers, E8. Track challenge progress and rule compliance automatically.'},
            ].map((f,i) => (
            ...
            <div className="lp-big-feat">
              <div>
                ...
                <ul className="lp-check-list">
                  {['Equity curve with drawdown analysis','Performance by session, pair & setup','Hour × day heatmap for optimal timing','Risk-adjusted returns (Sharpe, Sortino, Calmar)'].map((t,i)=><li key={i}><span className="lp-check-ico">✓</span>{t}</li>)}
                </ul>
              </div>
              <div className="lp-big-visual">
                ...
                <div className="lp-visual-body">
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
                    {[{l:'Win Rate',v:'68%',c:'#00FF88'},{l:'Profit Factor',v:'2.34',c:'#06E6FF'},{l:'Sharpe',v:'1.87',c:'#B06EFF'},{l:'Max DD',v:'-4.2%',c:'#FF3D57'}].map((m,i)=>(
                      <div key={i} style={{padding:'10px',borderRadius:8,background:'rgba(255,255,255,0.03)',border:'1px solid #162034'}}>
                        <div style={{fontSize:8,color:'#334566',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.8px',marginBottom:3}}>{m.l}</div>
                        <div style={{fontSize:16,fontWeight:800,fontFamily:"'JetBrains Mono',monospace",color:m.c}}>{m.v}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{padding:'10px',borderRadius:8,background:'rgba(255,255,255,0.02)',border:'1px solid #162034',height:120,display:'flex',alignItems:'flex-end',gap:3}}>
                    {[35,45,30,55,65,50,70,60,75,85,70,80,90,85,95].map((h,i)=>(
                      <div key={i} style={{flex:1,height:`${h}%`,borderRadius:'3px 3px 0 0',background:h>70?'rgba(0,255,136,0.3)':h>50?'rgba(6,230,255,0.2)':'rgba(255,255,255,0.06)'}}/>
                    ))}
                  </div>
                </div>
              </div>
          ...
          <Reveal>
            <div className="lp-big-feat reverse">
              <div>
                ...
                <ul className="lp-check-list">
                  {['Automatic bias detection (revenge trading, FOMO, overtrading)','Personalized improvement recommendations','Pattern recognition across hundreds of trades','Natural language chat — ask anything about your trading'].map((t,i)=><li key={i}><span className="lp-check-ico">✓</span>{t}</li>)}
                </ul>
              </div>
              <div className="lp-big-visual">
            ...
            {[
              ...
              <Reveal key={plan.id} delay={i*0.1}>
                <div className={`lp-pricing-card ${plan.popular?'popular':''}`}>
                  ...
                  <div className="lp-price"><sup>$</sup>{plan.price}</div>
                  <div className="lp-period">per month</div>
                  <div className="lp-save">14 days free trial</div>
            ...
            {TESTIS.map((t,i) => (
              <Reveal key={i} delay={i*0.08}>
                <div className="lp-testi-card">
                  ...
                  <div className="lp-testi-text" dangerouslySetInnerHTML={{__html:t.text}}/>
                  <div className="lp-testi-author">
                    <div className="lp-avatar" style={{background:t.grad}}>{t.i}</div>
                    <div><div className="lp-testi-name">{t.name}</div><div className="lp-testi-role">{t.role}</div><div className="lp-testi-pnl">{t.pnl}</div></div>
                  </div>
                </div>
              </Reveal>
            ))}
          ...
          <div className="lp-faq-list">
            {FAQS.map((f,i) => (
              <div key={i} className={`lp-faq-item ${openFaq===i?'open':''}`} onClick={()=>setOpenFaq(openFaq===i?null:i)}>
                <div className="lp-faq-q">{f.q}<span className="lp-faq-arrow">+</span></div>
      ...
      <section className="lp-cta">
        ...
        <Reveal delay={0.2}>
          <div className="lp-cta-actions">
            <button className="btn-hero-primary" onClick={onSignup}>Start your free trial →</button>
        ...
        <div className="lp-footer-inner">
          <div className="lp-footer-top">
            ...
            <div className="lp-footer-col">
              ...
              <a href="#features">Features</a>
              <a href="#pricing">Pricing</a>
              <a href="#" onClick={e=>{e.preventDefault();setModal('changelog')}}>Changelog</a>
              <a href="#" onClick={e=>{e.preventDefault();setModal('roadmap')}}>Roadmap</a>
            </div>
            <div className="lp-footer-col">
              <h4>Resources</h4>
              <a href="#" onClick={e=>{e.preventDefault();setModal('docs')}}>Documentation</a>
              <a href="#" onClick={e=>{e.preventDefault();setModal('api')}}>API Reference</a>
              <a href="#" onClick={e=>{e.preventDefault();setModal('tutoriels')}}>Tutorials</a>
            </div>
            <div className="lp-footer-col">
              <h4>Legal</h4>
              <a href="#" onClick={e=>{e.preventDefault();setModal('cgu')}}>Terms of Service</a>
              <a href="#" onClick={e=>{e.preventDefault();setModal('rgpd')}}>Privacy Policy</a>
              <a href="#" onClick={e=>{e.preventDefault();setModal('contact')}}>Contact</a>
            </div>
          </div>
          <div className="lp-footer-bottom">
            <p>© 2026 MarketFlow Journal. All rights reserved.</p>
            <div className="lp-social-row">
              <a href="https://twitter.com/marketflowjrl" target="_blank" rel="noopener noreferrer" className="lp-social-btn">𝕏</a>
              <a href="https://discord.gg/Cvh6H8yK8m" target="_blank" rel="noopener noreferrer" className="lp-social-btn">💬</a>
            </div>
          </div>
        </div>
      </footer>
