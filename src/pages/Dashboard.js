...
const Card = ({ children, style={}, glow=null, hover=false, custom=0, onClick }) => (
  <motion.div
    ...
    onClick={onClick}
    style={{
      background:'linear-gradient(145deg,rgba(12,20,34,0.95),rgba(8,14,26,0.98))',
...
const LiveTicker = () => {
  ...
  return (
    <motion.div initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} transition={{duration:0.4}}
      style={{marginBottom:12,padding:'10px 18px',borderRadius:12,background:'linear-gradient(90deg,rgba(0,255,136,0.06),rgba(6,230,255,0.04))',border:`1px solid ${C.green}25`,display:'flex',alignItems:'center',justifyContent:'space-between',gap:14}}>
      ...
      <div style={{display:'flex',gap:8,alignItems:'center'}}>
        <span style={{fontSize:8.5,color:C.t3}}>SL: 1.08180 · TP: 1.08680</span>
...
const JournalNote = () => (
  <Card custom={17} glow={C.warn} hover={false} style={{padding:'20px 20px'}}>
    ...
    <div style={{fontSize:10,color:C.t2,lineHeight:1.75,background:'rgba(255,255,255,0.02)',borderRadius:12,padding:'12px 14px',border:`1px solid ${C.brd}`,marginBottom:12,minHeight:70}}>
...
// ─── Rank Modal ──────────────────────────────────────────────────────────────
function RankModal({ score, rank, onClose }) {
  const countryRank = Math.max(1, Math.round(1000 - score * 8 + Math.random() * 50));
  const worldRank = Math.max(1, Math.round(50000 - score * 400 + Math.random() * 2000));
...
const TradingCalendar = () => {
  ...
  return (
    <>
      ...
      {/* Rank Modal */}
      <AnimatePresence>
        {rankModal && <RankModal score={score} rank={rank} onClose={()=>setRankModal(false)}/>}
      </AnimatePresence>
    </>
  );
};
  ...
  return (
    <DashboardDataCtx.Provider value={dashData}>
      <div style={{background:'var(--bg,#030508)',minHeight:'100%',width:'100%',fontFamily:"'SF Pro Display','Segoe UI',system-ui,sans-serif",color:'var(--t1,#E8EEFF)',position:'relative'}}>
        ...
        <div style={{position:'relative',zIndex:1,padding:'28px 28px 48px',width:'100%',boxSizing:'border-box'}}>
          ...
          <motion.div initial={{opacity:0,y:-12}} animate={{opacity:1,y:0}} transition={{duration:0.45}}
            ...
            <div style={{display:'flex',gap:10,alignItems:'center'}}>
              {/* Summary stats */}
              <div style={{padding:'8px 16px',borderRadius:10,background:'rgba(255,255,255,0.04)',border:`1px solid ${C.brd}`,display:'flex',gap:18,alignItems:'center'}}>
                <div style={{textAlign:'center'}}>
