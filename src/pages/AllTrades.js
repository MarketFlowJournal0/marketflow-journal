...
const SecTitle=({title,color=C.cyan})=>(<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,marginTop:4}}><span style={{fontSize:10,fontWeight:800,color:C.t3,letterSpacing:'1.5px',textTransform:'uppercase'}}>{title}</span><div style={{flex:1,height:1,background:`linear-gradient(90deg,${color}40,transparent)`}}/></div>);
...
const PsychoCard=({score})=>{
  ...
  return(<div style={{padding:'12px 14px',borderRadius:10,background:`linear-gradient(135deg,${color}10,transparent)`,border:`1px solid ${color}28`,marginBottom:12}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}><div><div style={{fontSize:10,color:C.t3,fontWeight:600}}>Psychological Score</div><div style={{fontSize:11,color,fontWeight:700}}>{label}</div></div><motion.div initial={{scale:0}} animate={{scale:1}} transition={{type:'spring',delay:0.3}} style={{fontSize:26,fontWeight:900,color,fontFamily:'monospace'}}>{s}</motion.div></div><div style={{height:5,borderRadius:3,backgroundColor:C.bgDeep,overflow:'hidden'}}><motion.div initial={{width:0}} animate={{width:`${s}%`}} transition={{duration:0.9,ease:[0.22,1,0.36,1],delay:0.2}} style={{height:'100%',borderRadius:3,background:`linear-gradient(90deg,${color},${color}88)`,boxShadow:`0 0 10px ${color}60`}}/></div></div>);
};
      ...
      <motion.div key="pnl" initial={{x:'100%',opacity:0.6}} animate={{x:0,opacity:1}} exit={{x:'100%',opacity:0}} transition={{type:'spring',stiffness:300,damping:34,mass:0.85}} style={{position:'fixed',top:0,right:0,bottom:0,width:420,backgroundColor:C.bgCard,borderLeft:`1px solid ${C.brdSoft}`,zIndex:201,display:'flex',flexDirection:'column',boxShadow:'-20px 0 70px rgba(0,0,0,0.55)',fontFamily:'system-ui,-apple-system,sans-serif'}}>
        ...
        <div style={{position:'relative',zIndex:1,padding:'18px 18px 14px',borderBottom:`1px solid ${C.brd}`,background:`linear-gradient(180deg,${C.bgHigh},${C.bgCard})`,flexShrink:0}}>
          ...
          <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:12}}>
            <Tag label={type==='Long'?'Long':'Short'} color={typeC} bg={`${typeC}14`}/>
            <Tag label={trade.session||'-'} color={C.cyan} bg="rgba(0,212,255,0.08)"/>
          ...
          <div style={{padding:'12px 14px',borderRadius:10,background:`linear-gradient(135deg,${pColor}14,${pColor}04)`,border:`1px solid ${pColor}28`,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <div>
        ...
        <div ref={panelRef} style={{flex:1,overflowY:'auto',overflowX:'hidden',padding:'18px',scrollbarWidth:'thin',scrollbarColor:`${C.brd} transparent`}}>
          ...
          {trade.notes&&(<><SecTitle title="Notes" color={C.warn}/><div style={{padding:'13px 15px',borderRadius:10,backgroundColor:C.bgDeep,border:`1px solid ${C.brd}`,borderLeft:`3px solid ${C.warn}40`,fontSize:12,color:C.t2,lineHeight:1.75,fontStyle:'italic',marginBottom:16,whiteSpace:'pre-wrap'}}>{trade.notes}</div></>)}
