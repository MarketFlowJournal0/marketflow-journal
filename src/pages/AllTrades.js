...
const C = {
  ...
  cyan:'#00D4FF', cyanGlow:'rgba(0,212,255,0.35)', teal:'#00C9A7',
  green:'#00E676', greenDim:'#10B981', greenGlow:'rgba(0,230,118,0.35)',
  danger:'#FF4757', dangerDim:'#EF4444', dangerGlow:'rgba(255,71,87,0.35)',
  warn:'#FFB300', purple:'#A78BFA', blue:'#5B7BF6',
...
// 🧩 UI ATOMS
// ══════════════════════════════════════════════════════════════════════════════
const Tag=({label,color,bg})=>(<span style={{display:'inline-flex',alignItems:'center',padding:'3px 9px',borderRadius:5,fontSize:11,fontWeight:700,whiteSpace:'nowrap',color,backgroundColor:bg,border:`1px solid ${color}30`}}>{label}</span>);

const GlassBtn=({children,onClick,variant='default',icon,disabled,loading,size='md',fullWidth})=>{
  const[ripples,setRipples]=useState([]);
  const V={default:{bg:C.bgGlass,border:C.brd,color:C.t2,shadow:'none'},primary:{bg:C.grad,border:'transparent',color:C.bgDeep,shadow:`0 6px 24px ${C.cyanGlow}`},danger:{bg:'rgba(255,71,87,0.1)',border:C.danger,color:C.danger,shadow:'none'},ghost:{bg:'transparent',border:'transparent',color:C.t2,shadow:'none'},cyan:{bg:'rgba(0,212,255,0.08)',border:C.cyan,color:C.cyan,shadow:'none'}};
...
const TradeRow=React.memo(({trade,isSelected,onSelect,onClickDetail,onDoubleClickEdit,cols,cumulativePnl})=>{
  ...
  const cell=(key)=>{
    switch(key){
      ...
      case 'type':return<Tag label={type==='Long'?'↗ Long':'↘ Short'} color={type==='Long'?C.green:C.danger} bg={type==='Long'?'rgba(0,230,118,0.1)':'rgba(255,71,87,0.1)'}/>;
  ...
  return(<motion.tr initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-6}} transition={{duration:0.15}} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} onClick={onClickDetail} onDoubleClick={()=>onDoubleClickEdit(trade)} style={{backgroundColor:rowBg,borderLeft:`2px solid ${isSelected?C.cyan:isWin?'rgba(0,230,118,0.15)':'rgba(255,71,87,0.15)'}`,cursor:'pointer',transition:'all 0.12s ease'}}>{cols.map(col=>(<td key={col.key} onClick={col.key==='select'?(e)=>{e.stopPropagation();onSelect(trade.id);}:undefined} style={{padding:'10px 14px',whiteSpace:'nowrap',borderBottom:`1px solid ${C.brd}`,verticalAlign:'middle'}}>{col.key==='select'?<input type="checkbox" checked={isSelected} onChange={()=>onSelect(trade.id)} onClick={e=>e.stopPropagation()} style={{cursor:'pointer',accentColor:C.cyan,width:14,height:14}}/>:cell(col.key)}</td>))}</motion.tr>);
});
...
// 🎯 TRADE DETAIL PANEL
// ══════════════════════════════════════════════════════════════════════════════
const PBar=({label,value,max=100,color})=>(<div style={{marginBottom:10}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}><span style={{fontSize:10,color:C.t3,fontWeight:600}}>{label}</span><span style={{fontSize:10,fontWeight:800,color}}>{value}</span></div><div style={{height:4,borderRadius:2,backgroundColor:C.bgDeep,overflow:'hidden'}}><motion.div initial={{width:0}} animate={{width:`${Math.min(100,(value/max)*100)}%`}} transition={{duration:0.8,ease:[0.22,1,0.36,1]}} style={{height:'100%',borderRadius:2,background:color,boxShadow:`0 0 8px ${color}50`}}/></div></div>);
const SecTitle=({icon,title,color=C.cyan})=>(<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,marginTop:4}}><span style={{fontSize:13}}>{icon}</span><span style={{fontSize:10,fontWeight:800,color:C.t3,letterSpacing:'1.5px',textTransform:'uppercase'}}>{title}</span><div style={{flex:1,height:1,background:`linear-gradient(90deg,${color}40,transparent)`}}/></div>);
const MRow=({label,value,color,sub,icon})=>(<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 0',borderBottom:`1px solid ${C.brd}`}}><div style={{display:'flex',alignItems:'center',gap:7}}>{icon&&<span style={{fontSize:13,opacity:0.8}}>{icon}</span>}<div><div style={{fontSize:10,color:C.t3,fontWeight:600,letterSpacing:'0.4px'}}>{label}</div>{sub&&<div style={{fontSize:9,color:C.t4,marginTop:1}}>{sub}</div>}</div></div><div style={{fontSize:13,fontWeight:800,color:color||C.t1,fontFamily:'monospace'}}>{value}</div></div>);
          ...
          {step===1&&(
            <div style={{padding:'22px',flex:1,overflowY:'auto'}}>
              <div onDragEnter={e=>{e.preventDefault();setDrag(true);}} onDragLeave={e=>{e.preventDefault();setDrag(false);}} onDragOver={e=>{e.preventDefault();setDrag(true);}} onDrop={e=>{e.preventDefault();setDrag(false);if(e.dataTransfer.files[0])handleFileLoad(e.dataTransfer.files[0]);}} onClick={()=>fileRef.current?.click()}
                ...
                {parsing?<div style={{fontSize:13,color:C.cyan}}><motion.div animate={{rotate:360}} transition={{duration:1,repeat:Infinity,ease:'linear'}} style={{width:26,height:26,border:`3px solid ${C.cyan}`,borderTopColor:'transparent',borderRadius:'50%',margin:'0 auto 10px'}}/>Analyse intelligente en cours…</div>
                :file?<><div style={{fontSize:38,marginBottom:8}}>✅</div><div style={{fontSize:13,fontWeight:700,color:C.green,marginBottom:3}}>{file.name}</div><div style={{fontSize:10,color:C.t3}}>{(file.size/1024).toFixed(1)} KB · Cliquez pour changer</div></>
                    ...
                    return(
                      <motion.div key={h} initial={{opacity:0,y:4}} animate={{opacity:1,y:0}}
                        style={{
                          ...
                          border:`1px solid ${isReq?`${C.cyan}60`:isIgn?C.brd+'40':isExtr?`${C.blue}30`:`${cc}30`}`,
                ...
                <div style={{marginTop:12,padding:'10px 14px',borderRadius:8,backgroundColor:'rgba(0,212,255,0.03)',border:`1px solid rgba(0,212,255,0.1)`}}>
                  ...
                  <div style={{display:'flex',flexWrap:'wrap',gap:4}}>
                    {KNOWN_FIELDS.filter(f=>f.value!=='_ignore'&&f.value!=='_extra').map(f=>{
                      ...
                      return(<div key={f.value} style={{fontSize:8,padding:'3px 8px',borderRadius:5,backgroundColor:isMapped?`${C.green}12`:C.bgDeep,border:`1px solid ${isMapped?C.greenDim:C.brd}`,color:isMapped?C.green:C.t4,fontWeight:isMapped?700:400,display:'flex',alignItems:'center',gap:4}}>{f.label.replace(/[📍↕️📈📉💰📅🕐🌍⚖️🛑✨⚡🔄🎯📝🧠📦💸🔁⚠️📰🏷️🏦👤⏱️📎]/g,'').trim()}{isMapped&&<span style={{color:C.teal,fontSize:7}}>← {mappedFrom}</span>}</div>);
                    })}
                  </div>
                </div>
              </div>
