...
function AppInner() {
  ...
  // Vérifier UNE SEULE FOIS si l'user a un abonnement Stripe
  useEffect(() => {
    if (loading)        return;
    if (!user?.id)      return;
  ...
  // Bloquer retour navigateur sur PlanSelection (une seule fois)
  useEffect(() => {
  ...
  const openSignupWithPlan = (priceId) => { sessionStorage.setItem('pending_price_id', priceId); setAuthModal('signup'); };
    ...
    <TradingProvider>
      <div style={{ display:'flex', minHeight:'100vh', backgroundColor:'var(--bg)', fontFamily:"'Inter',sans-serif" }}>
        <div style={{ position:'fixed', top:0, left:0, bottom:0, width:sidebarWidth, zIndex:100, transition:'width 0.30s cubic-bezier(0.4,0,0.2,1)' }}>
          <Sidebar currentPage={currentPage} setCurrentPage={setCurrentPage} collapsed={collapsed} setCollapsed={setCollapsed} user={user} onLogout={handleLogout} />
        </div>
