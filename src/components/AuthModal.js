...
const SC = ['', '#FF3D57', '#FF9500', '#D7B36A', '#00D2B8'];
...
function Input({ label, type = 'text', value, onChange, error, placeholder, autoComplete, autoFocus, icon, ...rest }) {
  ...
  return (
    <div style={{ position: 'relative', marginBottom: error ? 4 : 14 }}>
      <label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: error ? '#FF6B6B' : focused ? '#14C9E5' : 'rgba(255,255,255,0.35)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6, transition: 'color 0.2s' }}>{label}</label>
      <input
...
export default function AuthModal({ onClose, onSuccess, defaultTab = 'login' }) {
  ...
  return (
    <>
      <style>{`.auth-link { color: rgba(6,230,255,0.45); text-decoration: none; transition: color 0.15s; } .auth-link:hover { color: #14C9E5; }`}</style>
                    ...
                    <form onSubmit={handleForgot} noValidate>
                      {forgotSent ? (
                        <div style={{ textAlign: 'center', padding: '16px 0' }}>
                          ...
                          <button type="button" onClick={() => { setForgotMode(false); setForgotSent(false); }} style={{ width: '100%', padding: 12, background: 'linear-gradient(135deg, #14C9E5, #00D2B8)', border: 'none', borderRadius: 10, fontFamily: "'Space Grotesk',sans-serif", fontSize: 13, fontWeight: 700, color: '#01040A', cursor: 'pointer' }}>Back to Sign In</button>
                        </div>
                      ) : (
                        <>
